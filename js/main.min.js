...
b.Candidates.length+" candidate"+(1===b.Candidates.length?"":"s")+'</span> <span class="badge badge-secondary">'+b.NumberPositions+" position"+(1===b.NumberPositions?"":"s")+'</span></small></h4><ul class="list-group">'+b.Candidates.reduce(n,"")+"</ul>"},"")});r=!0;m.classList.remove("d-none");e.classList.add("d-none");c.value=a;k=a.toLowerCase();f=e.getElementsByTagName("button");for(d=0;d<f.length;d+=1)f[d].getAttribute("data-address")!==a&&(f[d].outerHTML="",--d)}function l(){var a=c.value.trim().toLowerCase();
if(""===a)k=a,e.innerHTML='<div class="list-group-item list-group-item-info">To get started, enter your address.<br /><em>i.e. 99 Foster Dr</em></div>';else if(k!==a)if(h&&0===a.indexOf(k))e.innerHTML='<div class="list-group-item list-group-item-danger"><strong>There are no addresses available.</strong><br />Be sure to use a complete residential address with the civic number first.</div>';else if(k=a,n.test(a.charAt(0))){var b=t.getElementsByTagName("i")[0];b.classList.add("fa-spinner");b.classList.remove("fa-times");
b.classList.add("fa-pulse");h=!1;$.get("voterView.asp",{method:"street_addresses",query:a},"json").done(function(a){if(0===a.length)e.innerHTML='<div class="list-group-item list-group-item-danger"><strong>There are no addresses available.</strong><br />Be sure to use a complete residential address with the civic number first.</div>',h=!0;else{var d=!1;e.innerHTML=a.reduce(function(a,b){if(null===b.StreetNumber)return a;d=!0;var c=(b.StreetName+" "+b.StreetType+" "+b.StreetDirection).trim();return a+
'<button class="list-group-item list-group-item-action" data-address="'+b.Address+'" data-street-number="'+b.StreetNumber+'" data-street-name="'+b.StreetName+'" data-street-name-full="'+c+'" data-ward="'+b.Ward+'" data-poll="'+b.PollAndSuffix+'" type="button"><div class="clearfix"><strong class="float-left">'+b.Address+"</strong>"+('<small class="float-right text-right">Ward '+b.Ward+"<br />Poll "+b.PollAndSuffix+"</small>")+"</div></button>"},"");if(d){a=e.getElementsByTagName("button");var c;for(c=
0;c<a.length;c+=1)a[c].addEventListener("click",u)}else e.innerHTML='<div class="list-group-item list-group-item-danger"><strong>There are no addresses available.</strong><br />Be sure to use a complete residential address with the civic number first.</div>'}b.classList.remove("fa-pulse");b.classList.add("fa-times");b.classList.remove("fa-spinner")}).fail(function(){try{window.console.log("An error occurred communicating with VoterView.")}catch(f){}})}else e.innerHTML='<div class="list-group-item list-group-item-danger">Be sure to use a complete residential address with the civic number first.</div>'}
